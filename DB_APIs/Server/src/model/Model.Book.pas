Unit Model.Book;

Interface

Uses
    MVCFramework.Serializer.Commons,
    MVCFramework.ActiveRecord,
    MVCFramework.Nullables,
    System.SysUtils;

Type
    [MVCNameCase(ncLowerCase)]
    [MVCTable('Book.Book')]
    TBook = class(TMVCActiveRecord)
    private
      [MVCTableField('BookID', [foPrimaryKey, foAutoGenerated])]
      FBookID: Int64;

      [MVCTableField('Title')]
      FTitle: string;

      [MVCTableField('AuthorID')]
      FAuthorID: NullableInt32;
      [MVCTableField('PublisherID')]
      FPublisherID: NullableInt32;
      [MVCTableField('GenreID1')]
      FGenreID1: NullableInt32;
      [MVCTableField('GenreID2')]
      FGenreID2: NullableInt32;
      [MVCTableField('GenreID3')]
      FGenreID3: NullableInt32;

      [MVCTableField('GenreExtra')]
      FGenreExtra: NullableString;

      [MVCTableField('Description')]
      FDescription: NullableString;

      [MVCTableField('PublishedYear')]
      FPublishedYear: NullableInt16;

      [MVCTableField('LanguageID')]
      FLanguageID: NullableInt32;

      [MVCTableField('PageCount')]
      FPageCount: NullableInt32;

      [MVCTableField('ISBN')]
      FISBN: NullableString;

      [MVCTableField('CreatedAt')]
      FCreatedAt: TDateTime;

      [MVCTableField('UpdatedAt')]
      FUpdatedAt: TDateTime;

    public
      constructor Create; override;

      Property BookID: Int64 read FBookID write FBookID;
      Property Title: string read FTitle write FTitle;
      Property AuthorID: NullableInt32 read FAuthorID write FAuthorID;
      Property PublisherID: NullableInt32 read FPublisherID write FPublisherID;
      Property GenreID1: NullableInt32 read FGenreID1 write FGenreID1;
      Property GenreID2: NullableInt32 read FGenreID2 write FGenreID2;
      Property GenreID3: NullableInt32 read FGenreID3 write FGenreID3;
      Property GenreExtra: NullableString read FGenreExtra write FGenreExtra;
      Property Description: NullableString read FDescription write FDescription;
      Property PublishedYear: NullableInt16 read FPublishedYear write FPublishedYear;
      Property LanguageID: NullableInt32 read FLanguageID write FLanguageID;
      Property PageCount: NullableInt32 read FPageCount write FPageCount;
      Property ISBN: NullableString read FISBN write FISBN;
      Property CreatedAt: TDateTime read FCreatedAt write FCreatedAt;
      Property UpdatedAt: TDateTime read FUpdatedAt write FUpdatedAt;
End;

Implementation

{ TBook }

constructor TBook.Create;
begin
  inherited;
  FCreatedAt := Now;
  FUpdatedAt := Now;
end;

End.
